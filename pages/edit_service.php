...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("UPDATE services SET title = ?, description = ?, price = ?, category = ?, location = ?, latitude = ?, longitude = ?, image = ?, status = ? WHERE id = ? AND provider_id = ?");
            $stmt->execute([$title, $description, $price, $category, $location, $latitude, $longitude, $imageName, $status, $serviceId, $_SESSION['user_id']]);
            $success = 'Service updated successfully!';
            // Refresh data
            $service = array_merge($service, compact('title', 'description', 'price', 'category', 'location', 'latitude', 'longitude', 'imageName', 'status'));
