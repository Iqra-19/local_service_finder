...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let pollInterval;
    let pollDelay = 4000; // start at 4s polling rate to optimize resource usage on shared hosting
    const minPollDelay = 4000;
    const maxPollDelay = 15000; // max backoff to 15s if no new messages
    ...
    function loadContacts(callback) {
        fetch('message_handler.php?action=fetch_contacts')
            .then(res => res.json())
            .then(data => {
                ...
                if (data.status === 'success') {
                    ...
                    if (activeContactId && !hasInitialInList) {
                        ...
                        fetch(`message_handler.php?action=get_contact_info&contact_id=${activeContactId}`)
                            .then(res => res.json())
                            .then(infoData => {
    ...
    function startPolling() {
        ...
        pollInterval = setInterval(fetchMessages, pollDelay);
    }
    ...
    function openChat(contactId, contactName, contactRole) {
        ...
        pollDelay = minPollDelay; // Reset poll rate to default when chat session begins
    ...
    function fetchMessages() {
        ...
        fetch(`message_handler.php?action=fetch&contact_id=${activeContactId}`)
            .then(res => res.json())
            .then(data => {
                if (data.status === 'success') {
                    ...
                    if (messages.length !== lastMessageCount) {
                        ...
                        // Reset poll rate to default fast setting if a new message was detected
                        if (pollDelay !== minPollDelay) {
                            pollDelay = minPollDelay;
                            startPolling();
                        }
                    } else {
                        // Back off polling speed gradually if no new messages are arriving
                        if (pollDelay < maxPollDelay) {
                            pollDelay = Math.min(maxPollDelay, pollDelay + 2000);
                            startPolling();
                        }
                    }
                }
    ...
    chatForm.addEventListener('submit', function(e) {
        ...
        .then(data => {
            if (data.status === 'success') {
                // Immediately refresh chat feed, reset polling speed, and reload sidebar
                pollDelay = minPollDelay;
                fetchMessages();
                loadContacts();
            } else {
    ...
    document.addEventListener('visibilitychange', function() {
        if (document.hidden) {
            ...
            if (activeContactId) {
                pollDelay = minPollDelay; // Fast polling on focus
                fetchMessages();
                startPolling();
            }
        }
